...
function deployLambda {
  ...
  if [ -f "${lambda_base_name}-deps.zip" ]; then
    echo "LOG - Found Lambda dependencies. Creating Lambda Layer."

    layer_version_arn=$(aws lambda publish-layer-version --layer-name "dependencies-${lambda_aws_name}" --zip-file fileb://./${lambda_base_name}-deps.zip --compatible-runtimes python3.8 | jq -r .LayerVersionArn)
  ...
  aws lambda update-function-code --function-name "${lambda_aws_name}" --zip-file fileb://./${lambda_base_name}.zip
